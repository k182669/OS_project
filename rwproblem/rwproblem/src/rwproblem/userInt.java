...
public class userInt extends JFrame implements Read {
	...
	public userInt() throws IOException {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// nr = new thrGroup(Thread.currentThread().getThreadGroup(),
				// "nr");
				int n = Thread.activeCount();
				// Thread[] thr = new Thread[n];
				// thrGroup.enumerate(thr);
				rwArr[n] = nr;
				// rwArr[n].start();
					//rwArr[n].start();
					// nr.start();
	...
	public void setReadNum(int n) {
		System.out.println(n);
		this.readNum += n;
	}

	@Override
	public int getReader_counter() {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public void setReader_counter(int reader_counter) {
		// TODO Auto-generated method stub

	}

	/*
	 * public reader_window newRead() throws IOException{ reader_window rw = new
	 * reader_window(); rw.setLocation(0, 0); rw.addWindowListener(new
	 * WindowAdapter() { public void windowClosing(WindowEvent event) {
	 * r.setReader_counter(r.getReader_counter()-1);
	 * System.out.println("sub="+r.getReader_counter()); rw.dispose(); } });
	 * return rw;
	 * 
	 * }
	 */

	private static final class thrGroup extends Thread {
		public thrGroup(ThreadGroup group, String name) {
			super(group, name);
		}
	}
